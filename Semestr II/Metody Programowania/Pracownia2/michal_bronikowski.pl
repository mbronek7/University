...
find_numer_of_current_clause(Clause, Proof, Number) :-  %predykat bierze numer klazuli ktora jest uzyta w dowodzie
   memberchk([Clause,Number,_],Proof).
...
format_proof(PROOF, RESULT, Temporary_List_To_Collect_Solution):-
  first_and_last_element(PROOF, [Clause, _, SourceClausules], ActualEndOfListOfClauses), % "_" to numer ktory był potrzebny predykatowi prove
